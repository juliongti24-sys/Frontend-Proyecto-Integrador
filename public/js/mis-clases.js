...
if(confirmarUnirse) {
  confirmarUnirse.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/v1/student/classes/join', {
        method: 'POST',
        headers: {
...
async function cargarClasesEstudiante(estudianteId) {
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/v1/student/classes/${estudianteId}`);
